    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex gap-1">{CATEGORIES.map(c => <button key={c} onClick={() => setCategory(c)} className={`${btnBase} ${category === c ? btnActive : btnInactive}`}>{CATEGORY_LABELS[c]}</button>)}</div>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">중요도</span>
          <div className="flex gap-1">{PRIORITIES.map(p => <button key={p} onClick={() => setPriority(p)} className={`${btnBase} ${priority === p ? btnActive : btnInactive}`}>{PRIORITY_EMOJI[p]} {PRIORITY_LABELS[p]}</button>)}</div>
        </div>
        <div className="flex items-center gap-1.5">
